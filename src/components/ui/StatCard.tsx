...
const accentMap = {
  blue:    { icon: 'bg-accent-600/15 text-accent-400', glow: 'group-hover:shadow-brand-sm' },
...
export function StatCard({ title, value, subtitle, icon, iconBg, trend, accent = 'blue' }: StatCardProps) {
  ...
  return (
    <div className={`glass rounded-2xl p-5 glass-hover group cursor-default transition-all duration-base ${ac.glow}`}>
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium text-fg-muted uppercase tracking-wider mb-2">{title}</p>
        ...
        <div className={`${iconBg ?? ac.icon} rounded-xl p-3 flex-shrink-0 ml-3 transition-transform duration-base group-hover:scale-110`}>
          {icon}
        </div>
      </div>
    </div>
