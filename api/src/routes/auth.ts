...
import { denyToken } from '../db/redis.js'
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post('/refresh', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    if (oldJti) {
      ...
      if (ttl > 0) await denyToken(oldJti, ttl)
    }
    const token = app.jwt.sign(
  ...
  app.patch('/password', { onRequest: [app.authenticate] }, async (req, reply) => {
    const { sub: userId } = req.user
    const body = z.object({
  ...
  app.post('/admin/reset-password', { onRequest: [app.authenticate] }, async (req, reply) => {
  ...
  // POST /auth/logout — revoke token in Redis denylist, client drops it
