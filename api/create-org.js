export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(`${supabaseUrl.replace(/\/+$/, '')}/functions/v1/create-org`, {
      method: 'POST',
      headers: {
    ...
    const message = error instanceof Error ? error.message : 'Failed to proxy create-org request'
