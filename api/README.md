...
| `EMAIL_FROM` | | `noreply@velo.app` | From address for transactional emails |
...
TOKEN=$(curl -s -X POST http://localhost:3001/auth/login \
  ...
  -d '{"email":"admin@velo.local","password":"Admin1234!"}' \
  | jq -r .token)
```
...
```ini
# /etc/systemd/system/velo-api.service
[Unit]
Description=Velo CRM API
After=network.target postgresql.service redis.service

[Service]
WorkingDirectory=/opt/velo-api
EnvironmentFile=/opt/velo-api/.env
