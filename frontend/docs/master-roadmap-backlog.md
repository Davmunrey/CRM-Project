...
| **Full product audit + fixes** (2026-06-14) | Shipped | E2E audit (route-wiring + auth sweep + per-domain logic workflows) → ~41 fixes / 7 commits, migrations 026–029. Highlights: tickets-auth, deal `closed_won`/`closed_lost` stage alignment, owner-name model (028), server-side bulk email, admin create-member endpoint, transport-independent sequence reply-detection, MFA QR, per-day booking availability, optimistic-rollback across 7 stores. See implementation-history §36. |
