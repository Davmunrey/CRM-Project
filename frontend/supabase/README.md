...
**Note: Authentication and realtime are now handled by n0crm-api (api/ subdirectory in monorepo). These Edge Functions are legacy integrations and should not be called from the frontend.**

Core CRM functionality has migrated to `n0crm-api` (Fastify 5 + PostgreSQL 16). The Edge Functions in this directory are **legacy** — the frontend no longer calls them directly.

## What moved to n0crm-api

| Was Supabase Edge Function | Now n0crm-api route | Status |
|----------------------------|--------------------|--------|
...
| Auth (register, login, password reset) | `/auth/*` | ✅ Fully migrated (JWT via n0crm-api) |
...
These are **optional** and can be migrated to n0crm-api background jobs (BullMQ) in a future release.

## Supabase SQL migrations (reference only)

`supabase/migrations/` contains the **historical** Supabase PostgreSQL schema used during development. **Not used by n0crm-api** — n0crm-api has its own migrations under `../api/migrations/`.
...
**Recommended:** Migrate to n0crm-api webhook handler (`api/src/routes/webhook-subscriptions.ts`). Supabase webhooks are no longer called from the CRM.

---

*Last updated: 2026-05-18 — Frontend migration complete; all Supabase Edge Functions replaced by n0crm-api routes. This directory retained for historical reference and archive purposes only.*
