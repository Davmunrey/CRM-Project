...
export function GmailCallback() {
  ...
  useEffect(() => {
    ...
    async function exchange() {
      ...
      if (oauthError) {
        const errPayload: GoogleOAuthMessagePayload = { ok: false, error: oauthError }
        if (window.opener) { postToOpener(errPayload); return }
      ...
      if (!code) {
        if (window.opener) { postToOpener({ ok: false, error: 'missing_code' }); return }
      ...
      if (!isLegacy && !returnedState) {
        if (window.opener) { postToOpener({ ok: false, error: 'missing_state' }); return }
      ...
      try {
        const data = await api.post<{ access_token?: string; expires_in?: number; email_address?: string; error?: string }>(
        ...
        if (errMsg || !data || typeof (data as { access_token?: string }).access_token !== 'string') {
          const raw = errMsg ?? 'exchange_failed'
          if (window.opener) { postToOpener({ ok: false, error: raw }); return }
        ...
        if (d.email_address) setGmailAddress(d.email_address)

        if (window.opener) { postToOpener({ ok: true, email: d.email_address }); return }

        toast.success(t.settings.gmailConnectionActive)
        navigate('/inbox', { replace: true })
      } catch {
        if (window.opener) { postToOpener({ ok: false, error: 'network' }); return }
        toast.error(t.errors.gmailConnectionError)
        navigate('/settings/integrations', { replace: true })
      }
    }
