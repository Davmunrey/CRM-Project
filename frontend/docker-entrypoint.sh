...
# Normalise VELO_API_URL (strip trailing slash) so we never produce //paths in proxy_pass.
...
API_HOST=$(echo "${VELO_API_URL}" | sed -E 's|^https?://||; s|/.*$||; s|:.*$||')
if [ -z "${API_HOST}" ]; then
  echo "ERROR: could not derive API_HOST from VELO_API_URL='${VELO_API_URL}'"
  exit 1
fi
export API_HOST

envsubst '${VELO_API_URL} ${API_HOST} ${NAMESERVER}' \
  < /etc/nginx/conf.d/default.conf.template \
  > /etc/nginx/conf.d/default.conf

echo "Velo CRM starting — proxying /api → ${VELO_API_URL} (SNI/Host: ${API_HOST}, resolver: ${NAMESERVER})"
